...
**Which chongdae wrote it.** Every record file says `written_by: chongdae <version>` — with `+g<sha>[-dirty]` when a working source wrote it — and every judgment (`created`, `confirmed`, `accepted`, `retried`, `dropped`, …) carries the version that recorded it beside its `at`: one run can span versions. `settings.chongdae.commit-records: false` (hunsu.json, or this machine's overlay in hunsu.local.json) writes records without committing them. `report` names any committed record written by a build that is not a release (`unreleased-writer`). A check given with this machine's paths is recorded portably — an installed plugin's path as `{plugin:NAME}`, the home as `~` — and resolved again where it runs.
