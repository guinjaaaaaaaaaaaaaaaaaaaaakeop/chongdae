...
| `/chongdae:report` | What this record says a reviewer should see, typed `dwitbuk/findings@1`: outside-run, unattributed, delegated, stage-finding, verifier-reject (every rejected attempt's findings), left-open, non-claims, unreleased-writer (a committed record a working source wrote) — with `--since REV`, changes since that revision and the runs whose record was not complete there (earlier runs were the previous review's). Declared as a reporter in hunsu.json; dwitbuk collects it without knowing chongdae |
...
| `.chongdae/run-<id>/tasks/<id>.json` | yes | one file per task (so two people's tasks in one run merge as distinct files): status, claim, touched, response, review, `stages` (the before/after roles' answers and who gave them), attempts, confirmation, non-claims, `performed_by` and `verified_by` — and for session tasks the task's own definition |
...
| `.chongdae/run-<id>/<tag>.trace.json` | yes | what a worker did, from its session: the model, the files it changed (and read, where the host says), how many commands it ran and how many failed — the project as `.`, home as `~`. A model's run does not replay from its command lines; they are in `local/` |
| `.chongdae/run-<id>/<tag>.asked.json` | yes | what a provider was asked — the request with this machine taken out (`.`, `~`, `<plugin:…>`, `<tmp>`): what another person needs to put the same question to a worker again |
| `.chongdae/run-<id>/local/` | no | this machine's part of the record: the host sessions and transcripts behind each author line, each task's `start` snapshot, full worker traces with their command lines. The engine reads it back here; nobody else can open a session or needs a snapshot once `touched` is written |
| `.chongdae/run-<id>/<tag>.response.transcript.jsonl`, `<tag>.request.json`, `<tag>.response.json`, `*.provider.log`, `*.pending.json` | no (`.chongdae/.gitignore`, written by chongdae) | the worker's whole session, the request as sent and its raw answer — this machine's paths, often megabytes; kept here for a local audit, summarized into the trace, the answer carried by the task file. A copy an earlier version committed leaves the index at the next record commit |
...
**Which chongdae wrote it.** Every record file says `written_by: chongdae <version>` — with `+g<sha>[-dirty]` when a working source wrote it — and every judgment (`created`, `confirmed`, `accepted`, `retried`, `dropped`, …) carries the version that recorded it beside its `at`: one run can span versions. While the project tries working sources (`hunsu dev`, recorded in `hunsu.local.json`), records are written and not committed — a trial leaves nothing in the project; its commit is the released version's. `report` names any committed record a working source wrote (`unreleased-writer`). A check given with this machine's paths is recorded portably — an installed plugin's path as `{plugin:NAME}`, the home as `~` — and resolved again where it runs.

**Who did it** is recorded on every produced task like a commit's author line. A session task: `performed_by: {provider: session, agent: <host_version_kind from AI_AGENT>, effort, model}` (the host session id stays in `local/`) — the host does not tell its subprocesses the model, so it comes from the session's transcript (the SessionStart hook records where it is; Codex: the thread's rollout); a session the host did not persist gets `model-unknown` with the reason, never a guess. A command task: `{provider: command, argv: <the un-resolved argv>, host, model, turns, cost_usd}` (its session and transcript in `local/`) — the argv is portable, the rest is the worker's own account of its call (`worker` in its response). A verifier's call is recorded the same way as `verified_by`.

**The run is the agent's container.** The plugin's PreToolUse hook refuses Edit/Write in a project with no run in progress (the record, `.claude/` and `hunsu*` are exempt — their own engines write them); the SessionStart line says which run is open. So runs must be cheap: `init --session` opens one with no plan, `add <id>` names each piece as the work starts (a task with no check is allowed — done is then the agent's word, recorded as a non-claim), `run` records what each piece touched, `close` ends it. A task's word stays its own: when a task is done, the files it recorded are measured from there for the session tasks still open, and a no-check task that changed nothing of its own is not swept to done by the same `run` right after the task before it. A session build's `--tests` must exist when it is added (its start is taken then); write them in their own task first. Bash writes are not covered; dwitbuk's `outside-run` catches them. A task claimed by someone else (`claim`) is not this machine's to advance.

**What a project does by its own procedure** is not a run's. A site's posts are its author's, written by the project's own guide, and the pages built from them follow; a run around each would put the author's writing in the builders' ledger. The project declares those paths once, in hunsu.json `settings: {"chongdae": {"outside-runs": ["content/", "docs/"]}}` (read there, not from the lock, so it holds while a working source is tried with `hunsu dev`): project-relative files or directories, an entry that leaves the project names nothing. The write hook lets them through without a run, and `report` leaves them out of `outside-run` — saying so in one `non-claim` with how many files it left out, so a reviewer sees the boundary rather than a silence.
